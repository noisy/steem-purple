...
gboolean is_valid_email(gchar *email) {
	...
	return string_is_match(email, "([0-9a-zA-Z\\_]+[0-9a-zA-Z\\-\\.\\_]*)@(uin.icq|mail.ru|mail.ua|list.ru|inbox.ru|bk.ru|biz.mail.ru|corp.mail.ru)");
}
...
gboolean is_myworld_able(gchar *email) {
	...
	return string_is_match(email, "([0-9a-zA-Z\\_]+[0-9a-zA-Z\\-\\.\\_]*)@(mail.ru|mail.ua|list.ru|inbox.ru|bk.ru|biz.mail.ru|corp.mail.ru)");
}
...
gchar *mrim_get_ua_alias(MrimData *mrim, gchar *ua) {
	g_return_val_if_fail(mrim != NULL, NULL);
	g_return_val_if_fail(mrim->gc != NULL, NULL);
	g_return_val_if_fail(mrim->gc->account != NULL, NULL);
